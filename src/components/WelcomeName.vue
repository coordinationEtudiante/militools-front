...
const hello = computed(() => {
  if (time >= 9 && time < 12) {
    return t('hello.morning', { name })
  } else if (time >= 12 && time < 14) {
    return t('hello.noon', { name })
  } else if (time >= 14 && time < 19) {
    return t('hello.afternoon', { name })
  } else if (time >= 19 && time < 22) {
    return t('hello.evening', { name })
  } else if (time >= 22 || time < 7) {
    return t('hello.night', { name })
  } else {
    /*(time >= 7 && time < 9)*/
    return t('hello.awakening', { name })
  }
})
</script>
